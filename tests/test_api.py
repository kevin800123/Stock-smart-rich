...
def test_public_overview_shares_internal_frontend(tmp_path, monkeypatch):
    ...
    assert 'src="/app.js?v=20260817-ui66"' in html.text
    assert 'href="/styles.css?v=20260817-ui66"' in html.text
    assert 'src="app.js?v=20260817-ui66"' not in html.text
    assert 'href="styles.css?v=20260817-ui66"' not in html.text
...
def test_settings_rejects_data_dir_outside_root(tmp_path, monkeypatch):
    ...
    app = create_app()
    client = TestClient(app)
    bad = "/etc" if os.name != "nt" else r"C:\Windows"
...
def test_cup_handle_screen_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(tpex, "fetch_otc_names", lambda: {})   # 沒樁會真的去櫃買抓名稱
    app = create_app()
    client = TestClient(app)
