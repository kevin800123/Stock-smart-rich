...
def test_fetch_drops_a_day_that_only_has_night_rows_from_a_multi_day_range(monkeypatch):
    """排程實際會打的是重疊補最近幾個交易日的多日區間，不是單日——`refresh_ssf_daily`
    抓的是 `[D-6, D]` 約 7 個日曆天，正常只含 5 個交易日。

    5 個完整交易日 × 1,629 列 ＋ 今天只有盤後列（一般 0 列）：若用整個回應加總，
    8,145 列遠超門檻，守衛會誤判整批通過，今天那半天的資料就被寫進 DB。
