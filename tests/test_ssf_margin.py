...
def test_margin_uses_round_half_up_not_bankers_rounding():
    ...
    **契約與文字修正（review #1）**：原本第二個例子（小型台積電 33,115.5）其實
    ...
    「這兩個各會少 1 元」因此不實（只有 96592.5 那個會）。換成小型緯穎 PWF
    的半元案例（42,808.5，42808 是偶數；同「保證金四條紀律」規則 1 已經記載
    過的券商實測數字）——ROUND_HALF_UP 進到奇數的 42809，`round()` 銀行家
    進位會退回偶數的 42808，兩者才真的不同，兩個例子合起來才都是
    load-bearing 的。
    ...
    assert ssf.margin_amount(2114, 100, 20.25) == 42809        # 小型緯穎 PWF，42,808.5
