...
### 自算選股「新進榜」標籤：new／燙金 Week NEW（ui53，2026-09）

使用者要自算選股表格裡新進榜的個股，股名後面掛標籤。兩種，**判定都在後端、只看已記錄的
正式名單（`signal_ledger` source=`self_screen`）**，不重算前一天的全市場自算：

- **new（冷藍描邊）**：`ledger.previous_self_screen_codes(conn, 當天)`＝`signal_date < 當天`
  最近一個有記錄的名單。取「有記錄的最近一天」而不是日曆前一天：週末、假日、排程漏跑的那天
  本來就沒有名單。
- **Week NEW（燙金，使用者指定）**：`ledger.previous_custody_week_codes`＝**上一個集保週期**
  內所有名單的代號聯集。大戶增比／人數降比一週才變一次，所以「集保換週後才進榜」要對照整個
  上一週期，不是前一天。週期以 `custody_dist` 的週日期（週五）為界，**從週五隔天起算**：週五
  那份名單是 17:30~19:30 提早算的，那時新一週集保通常還沒進來，用的是舊資料，歸在前一週期
  才對。週日期沿用 `custody_compare_weeks`（略過逐檔回補造成的殘缺週），且只看當天前一天以前
  的週。已知小瑕疵：週五晚上 21:00 用新集保重算後當晚打開頁面，比對的仍是前一週期。
- **沒有比對基準就一檔都不標**（`new_vs`／`week_new_vs` 為 null）：分不出新舊時標滿 new 等於
  沒標。同時符合只掛 Week NEW（使用者決定），表頭計數也照畫面上的標籤數。交叉檢視（關掉條件）
  時仍對照正式名單。端點回 `is_new`／`is_week_new`／`new_vs`／`week_new_vs`／計數。
- **視覺**：new 是註記，用全站「狀態＝藍」的小描邊標；Week NEW 是一週一次的重點，做成金屬箔片
  （多段金色漸層＋上緣切光＋下緣壓痕＋深墨字的壓印字影＋帶偏移軟陰影）。**不違反「琥珀只給
  注意這格」**：那個語彙是外框，燙金是實心箔面，軸不同。光掃過箔面**只在資料剛載入時播一次**
  （`ssBadgeFresh` → table 加 `.ss-badge-fresh`），排序／篩選重畫不再播；減少動畫偏好由全站
  那條 media query 收斂。
- **手機（≤600px）標籤換到股名下一行**：同一行會把凍結的第一欄撐到 179px（可視寬 350px 的
  一半），換行後回到 110px，只有帶標籤的列變高（39→58px）。
- **測試**：兩條端點測試（前一天名單、當天與其他來源不算；集保週期邊界含週五、不含前一個週日期）。
  本機 DB 沒有自算名單，UI 以注入假資料實測桌機／375px。
