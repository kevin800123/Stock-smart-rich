...
- `ss_trader.py`: rule engine for the「操盤手」view — quantifiable subset of the "Ss" trader methodology (full distillation lives in `.claude/skills/ss-trader/SKILL.md`): market checklist (融資維持率 13X% 抄底區, 融資 vs 大盤 wash, VIX contrarian, USD/TWD via the `twd` intl ticker, volume×position, 小那 vs 小道 fund flow, night-session ratio, settlement week) + 一紅吃三黑 candle signal + 季季高-approx picks. Served by `GET /api/ss-trader`; outputs are rule-match displays with a mandatory disclaimer, never advice.
