...
> **實作後修正（審查發現，已套用於 commit 048a7ae）**：守衛 3 原本數的是**整個回應**的
> 一般列數，但這支函式吃的是**日期區間**，而 Task 9 的排程請求的是 6 天區間。
> 13 個完整日（各 1,629 列）＋ 今天只有夜盤（0 列）＝ 總數 21,177 遠超門檻 → 守衛不會觸發，
> 今天那筆夜盤資料就被當成正常交易日寫進去——**正是這道守衛要擋的事**。
> 已改成**逐交易日各自判定**：依 `date` 分組、每天用自己的一般非價差列數對門檻，
> 只回合格日期的列（保持原順序），一天都不合格才回 `[]`，並記下被丟掉的日期與列數。
> 單日請求的行為完全不變。另外守衛 1、2 原本的反證不成立（會被下游守衛吸收），
> 已各補一條「只有該守衛能擋」的隔離測試。連線錯誤**刻意往上拋不吞**（見下方 Step 3 的
> docstring 與 CLAUDE.md 月營收那條教訓）。
...
## Task 6: 修復完成（2026-09-18）

> **完成者**: Claude Haiku 4.5  
> **Commit SHA**: ac24c88f263ee45642825bbe0574a742316e5d4d  
> **提交訊息**: 改進合約對照表測試，驗證 HTTP 錯誤日誌；用常數替換硬編碼門檻

### 修復項目

#### Fix 1: 改進測試驗證日誌記錄
**檔案**: `tests/test_ssf.py`  
**測試**: `test_fetch_contract_map_returns_empty_on_non_2xx_without_raising`

**修改前的問題**: 
- 測試只斷言結果是 `{}`，未驗證例外路徑是否被執行
- 即使 `raise_for_status()` 沒有被呼叫，測試仍會通過
- 無法檢測「設置正確的日誌訊息」

**修改內容**:
- 新增 `caplog` fixture 以捕捉日誌記錄
- 驗證當伺服器回傳 500 狀態碼時，日誌中會出現「抓取失敗」與「500」
- 修正 monkeypatch 策略：直接替換 `ssf` 模組的 `httpx` 物件，而非 `ssf.httpx.Client`
- 修改 `_ListResp.raise_for_status()` 實現，拋出簡單 `Exception` 而非 `HTTPStatusError`

**測試驗證**:
```bash
$ pytest tests/test_ssf.py::test_fetch_contract_map_returns_empty_on_non_2xx_without_raising -q
PASSED
```

#### Fix 2: 用常數替換硬編碼的 300 值
**檔案**: `stocks_power_rich/api/helpers.py`  
**函式**: `_ssf_contracts()`

**修改內容**:
- 行 161: `len(m) < 300` → `len(m) < taifex_ssf.MIN_PLAUSIBLE_CONTRACTS`
- 行 163: `len(fresh) >= 300` → `len(fresh) >= taifex_ssf.MIN_PLAUSIBLE_CONTRACTS`
- 文件字符串也更新為指向常數名稱而非硬編碼值

**好處**:
- 單一權威來源：常數定義在 `stocks_power_rich/sources/taifex_ssf.py` 第 261 行
- 避免常數重複定義，減少維護成本
- 若未來需要調整門檻，只需改一個地方
...
```bash
$ pytest tests/test_ssf.py -q
39 passed in 0.62s
```

所有 SSF 模組測試通過，包括新添加的日誌驗證測試。

### 文件更新

檔案 `docs/superpowers/plans/2026-09-18-stock-futures-overview.md` 已更新，記錄此修復之完成情況。
