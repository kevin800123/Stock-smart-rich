...
- **覆蓋表 `complete` 是黏的（2026-09-17）**：21:00 `run_update` 會對同一天再跑 `update_day`，櫃買行情重抓失敗時原本把 17:30 的 `complete` 覆寫成 `failed`（資料其實都在），21:00 自算選股重算因此連兩晚 `data_not_ready`、「融資3日」天天空白。`set_stock_source_coverage` 現在不讓 `failed`／`holiday` 蓋掉 `complete`，`attempts`／`last_error` 照記。21:00 只有櫃買**行情**（~1.7MB）失敗、法人與融資正常，同 09-12／13 大檔傳到一半被切的型；此端點不支援 Range，續傳用不上——**2026-09-23 改三層**：`fetch_otc_daily(strict=True)` 讓原因進覆蓋表 `last_error`、`QUOTE_RETRY_DELAYS=(2,5)` 重試、上櫃退到 openapi `tpex_mainboard_daily_close_quotes`（靜態檔可 `get_resumable` 續傳、只有最新交易日、日期不符回空），`heal_recent_quotes` 掛在 `run_update`，只重抓近 21 天內 `market_daily` 有收盤指數的失敗日（假日會標 failed，不濾會吃光名額）、每晚最多 2 個、重試不問備援、補不回來不進 failed（免得連發告警）；重試與備援只在每日路徑開，手動 `backfill` 兩者都不開（同步請求怕 502）；細節見 CLAUDE.md 同名段落（原本記載的後果是「上櫃維持率當晚算不出、靠 heal 補」——`_compute_otc_margin_maintenance`／`_heal_margin_maintenance` 已於 2026-09 移除，不再適用；仍受影響的是 `otc_margin_value`，見 CLAUDE.md ui70 那節）。
...
主圖「近 N 日類股漲跌表」從 `bfd0a53`（拆 APIRouter）起就壞了——後端把 `sectors` 從陣列改成 dict、前端沒跟，永遠「尚無類股資料」。直接移除 `/api/sectors/rotation`，換成 `GET /api/sectors/flow`：X＝法人近 5 日淨買賣 ÷ 類股市值、Y＝大戶 400張↑ 週增 ÷ 類股市值（除以市值才跨類股可比），泡泡＝類股、尾巴＝上一期→本期；純函式 `analysis.sector_flow`，大戶金額只有一份算式 `analysis.big_holder_amount`（selfcheck 也改呼叫它，`inspect.getsource` 鎖住）。大戶缺某檔 Δ 不進分子仍進分母、整類股沒 Δ 時 `y=None` 不是 0；法人交易日曆用 `stock_flow_daily` 自己的日期（不用 `market_daily`，今天的空列會被算進窗口）；快取鍵 `sectorflow:v3:{法人最新日}:{窗口指紋}:{全部集保週}:{days}`——**鍵要描述每一個輸入**：只放最新週會在 2→3 週時吃到舊快取（`has_custody` 讀取守衛是死碼、已拿掉），只放最新日則看不出「回補補進窗口中段一天」與「上櫃單邊重抓」，故加 `db.stock_flow_fingerprint`（逐日 COUNT＋Σ淨額的 md5 前 10 碼）——它結構上跑在快取查詢之前、連命中那次都要付（實測 days=5 掃 18 萬列 ~100ms、days=20 掃 64 萬列 ~341ms），**明知並接受**：一次進頁面只打一次，而改用 `stock_source_coverage` 當指紋雖只要 ~0.1ms，卻會把快取正確性綁在一張直接寫入者（含測試）不會更新的側表上；集保週用 `as_of=法人最新日` 挑，且第 2、3 個完整週**不保證相鄰**（殘缺週被略過，真實 DB 是 09-18／09-11／08-21），`_weeks_adjacent`（≤10 天）兩段都成立才給 `y_prev`，否則 `custody_prev_skipped="weeks_not_adjacent"`＋`custody_prev_gap` 讓說明列講出「跨 N 週、尾巴省略」；顏色一律 `C.info` 不碰紅綠；四區排行 `<button aria-pressed>` 是 canvas 的鍵盤替代，點泡泡或列篩下方交叉選股（ui72 起泡泡改依象限色、四區排行改象限領先者 Top 3，見下方 ui72 一節）；集保不足兩週降級成單軸長條。`custody_compare_weeks` 加 `limit`（預設 2）。

## 族群輪動重新設計（ui72，2026-09）
只改前端。`flowModel(d)` 是唯一的判定來源（放大鏡範圍、正規化刻度、三卡、常駐標籤、象限 Top 3、主圖範圍），所有區塊只讀它。放大鏡＝兩軸同比例修剪兩端、框住至少 80% 的最小範圍（含原點、加 12%；2026-09-23 修 8%、框 28/34）；正規化用放大鏡寬度（主圖全範圍會被單一離群值撐開、讓 X 主導）；加速轉強／風險外流要求本期 `comp` 同號且 `move` 同號。「沒有上期」分三種原因、卡片與 chip 各自對應（法人不足 2N 日（N＝法人窗口）／本期或上期集保跨 N 週／集保僅兩個完整週）。常駐標籤最多 8 個——**ECharts 會把 item 層 `label: { show: false }` 抄進該 item 的 `emphasis.label.show`、蓋掉 series 的 `emphasis.label.show: true`**，所以只在要常駐時才掛 item `label`；向量上期○→本期●、箭頭在中點，選取時其餘降到 25%（`FLOW_FADE = 0.25`）；tooltip 壓到 ≤300px（實測最寬 244.8px；放大鏡在 375／1024px 只有約 350px，`confine` 塞不下）；象限色 token `--flow-in/big/inst/out`，紅綠只在 tooltip 當日漲跌。百分比用 `flowPct` 固定兩位（`fmt` 會吃掉尾端 0）；`--fs-table` 不存在（只有 `--font-size-table`，未定義會安靜退回繼承的 16px）；高度要寫 `#view-rotation .flow-chart`（`.chart` 同權重在後面）。chips 在 `#flow-chip-list`（`role="status" aria-live="polite"`）裡、「ⓘ 指標說明」是旁邊的靜態按鈕——只清 `#flow-chip-list`、絕不清 `#flow-chips`；選取重畫（`renderFlowAll`）不含頁首；摘要卡用 `data-card` 找回焦點（同一類股可能同時是兩張卡）；focus 高亮＋`showTip` 只在 `:focus-visible` 時做（Chrome 滑鼠點按鈕也給焦點，否則 tooltip 會釘住）；失敗／無資料訊息同時寫進 `#flow-headline`（圖是 `role="img"`）；「○ 上期 → ● 本期」兩個圖例一律經 `setFlowLegends`、只在 `hasPrevAny`（真的畫了向量）時出現，清空的 `#flow-detail` 以 `:empty` 收掉。標籤右側放不下就翻到泡泡左側、右對齊（`flowLabelLayout`，常駐／選取／hover 都走它）：繪圖區右緣要在 callback 當下向圖表實例要（resize 只呼叫 `chart.resize()`），判斷要從泡泡框 `p.rect` 推算——`p.labelRect` 含 ECharts 留在 `textConfig.offset` 的上次 `dx`，拿它判斷每次 resize 都會左右來回翻；右側判斷與左側間距都算進 hover 放大的 `r×0.12`。`moveOverlap: "shiftY"` 對附著在泡泡上的標籤實測是 no-op（zrender 依 position 重算、蓋掉 `label.y`），標籤互相重疊是既有狀況。象限標題與放大鏡標題 `flex-wrap`＋名稱 `nowrap`，不再從字中間斷；頁首判讀 `text-wrap: pretty`。全寬度驗證修兩件：1181px 右欄比主圖高 120px（象限列折成三行）→ `.flow-quadrants` 開容器查詢 `flow-quads`、≤390px 收內距與欄距，修完 1560／1280／1181 都是 +23px；601px「風險外流」被擠成一字一行 → `.flow-card-top` 可換行、標題 `nowrap`。驗證時預覽頁是 `visibilityState=hidden`（ECharts 狀態與 tooltip 動畫要先截圖逼出繪製才生效）、`resize_window` 不觸發 `resize` 事件（要手動 dispatch）。**pre-merge polish（2026-09-24）**：頁面用請求序號（同全域搜尋 `gsSeq`）捨棄慢到的舊回應，載入失敗時「ⓘ 指標說明」按鈕改成停用而不是打開一個空面板。細節見 CLAUDE.md 同名段落。

## 融資維持率改用證交所官方整戶擔保維持率（ui70，2026-09）
自算的上市／上櫃維持率整條路徑刪除（`_compute_margin_maintenance`／heal／`analysis.margin_maintenance`／heal 端點），改存 `BFIJ3U` 的 `keep_rate`／`below_call_acc`／`call_acc`／`exe_acc`／`credit_amt` 與 `MI_MARGN_TREND` 的 `market_value`（2026-08-03 起、TREND 60 天上限、只填 NULL 回補、當日沒有記 `twse_credit` 尚未公布不告警；`MI_MARGN_TREND` 要錨在資料庫裡目前最新的那一列，不是 `date.today()`——錨在未來日期整段回錯誤 `stat`）。融資占市值＝融資金額÷上市總市值、**信用交易占比分母乘 2**（證交所 JS 原式），兩者讀取時算不落地。兩平線改每日加權（`ss_trader.blended_margin_ratio`，上市 0.6／上櫃 0.5 依融資金額加權），`otc_margin_value` 仍每天寫。台股大盤組 12 張、固定 4 欄（≤1240 3／≤1100 2；每個斷點剛好填滿）——ui70 的 10 張 5 欄在 1641px 以下退 4 欄會空 2 格，ui71 補「券資比」（`analysis.short_margin_ratio`）與「融資 5 日增減」（`analysis.rolling_change`，窗口 `ss_trader.MARGIN_CHG_DAYS`），`dashboard()` 逐列注入 `short_margin_ratio`／`margin_chg5` 不落地。四張新卡都帶 `dir-neutral`、不著紅綠；追繳壓力的琥珀外框只在高尾（位階 ≥90%）觸發。`GET /api/credit/backfill?days=60` 回 `filled`/`remaining`/`since`/`errors`，`remaining` 只數 `keep_rate` 的洞且不含今天；每次最多 30 個日期、日期之間 `_CREDIT_THROTTLE` 0.3 秒，重複呼叫直到 `remaining` 不再下降。`short_mv`／`margin_mv`／`otc_*_mv` 已不再寫值：融券卡副標拿掉、對照圖「融資／融券」窗格右軸改畫官方 `short_balance`（張）；LINE 21:00 卡片的整戶維持率由 `_daily_messages` 退到最近一筆 `keep_rate` 有值的列並傳 `keep_rate_asof` 標「截至 MM-DD」（BFIJ3U 多半晚於 21:00 產製）。此機器是 Python 3.14，`date.today` 不可 monkeypatch，測試改用 `_FrozenDate` 子類別換掉 `updater._date`。細節見 CLAUDE.md 同名段落。
