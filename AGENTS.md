...
RuntimeError（run_job 記 failed），不再回 `not_stored`。週六週報 18:00–21:30 每 30 分鐘，本週集保到了
...
一行集保說明（「集保 09-11→09-18」／「集保仍為 09-11 週（本週集保尚未取得）」／「本週集保尚未取得」；
寫「尚未取得」不寫「尚未公布」，程式不知道 TDCC 有沒有公布）。前瞻紀錄只在訊號日當天寫。
新進榜比對基準＝帳本 ∪ 使用者看過的名單（`refresh_self_screen_cache` 每次寫快取都記 `ai_cache
selfscreen_shown:{date}`、同日聯集、不論 record_signals；finding #4）：否則週六用新集保重算後已列成本週新進
的股，週一會被帳本裡的舊週五名單再報成「✦ 今天才進」。前瞻紀錄不受影響。查鍵用字典序範圍不用 LIKE（`_`
是萬用字元）；空名單不寫（帳本 0 檔那天也沒有列，兩邊要一致）。
`slot_times` 支援 `minute="0,30"`（`_cron_minutes`）。頁面覆蓋率列標「集保 前週→本週・取得時間」，只有
一週完整時標「尚無前一週可比」。已知、刻意不修：TDCC 若在週五 17:30～19:30 就公布，週五前瞻紀錄會用到
收盤後的新集保（實測週六才有，且原本 21:00 路徑同樣依賴時序）；全新空庫只有「補歷史」單檔週時完整週
...
`is_new`＝前一份使用者看過的自算名單（`signal_ledger` ∪ `selfscreen_shown:{date}`，見 custody_watch 段）沒有這檔；`is_week_new`＝**上一個集保週期**
