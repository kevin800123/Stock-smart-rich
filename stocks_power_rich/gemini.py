...
def summarize_news(payload: dict, api_key: str) -> dict:
    ...
    prompt = (
        ...
        "【選材優先序】10 則必須是**十件不同的事**。同一段行情（開盤跌幾點、盤中跌破某關卡、"
        ...
        "「市場情緒」這種放諸四海皆準的空話。\n\n"
        "本報告的每個市場都要恰好 10 則新聞：🇹🇼 台股 10 則、🇯🇵 日股 10 則、🇺🇸 美股 10 則。"
        "每則僅能使用對應市場清單的內容；若該市場少於 10 則，列出全部可用項目並註明來源不足，絕不可湊數。"
        ...
        "* 🔥 **[台股新聞標題]**\n"
        "  * 🧾 **事件**：來源可支持的核心事實。\n"
        "  * 🔢 **關鍵數據**：從**該則原始新聞標題**挑出「你上面那行標題沒寫進去」的量化事實"
        "（金額、張數、百分比、口數、倍數、日期），寫成含單位與比較基準的短句，多項用「、」分隔。"
        "把標題壓成 28 字時被你捨棄的數字，這一格就是用來補回來的。"
        "不得複述你自己標題裡已經有的數字，也不得複述加權指數、成交金額、台指期、日經、費半、VIX"
        "（這些程式已在訊息最上方列出，重複只會自相矛盾）。"
        "原始標題完全沒有數字時寫『無』，絕不可自行推算、換算或補造。\n"
        "  * 📈 **影響**：可能影響的市場、產業或情緒。\n"
        "  * 👀 **關注**：後續數據、政策、財報或價格反應。\n"
        "（共 10 則）\n\n"
        "#### 🇯🇵 日股｜10 則精選（株探翻譯）\n"
        "（沿用完全相同的 10 則格式；每個日文標題先翻成繁體中文）\n\n"
        "#### 🇺🇸 美股｜10 則精選\n"
        "（沿用完全相同的 10 則格式）\n\n"
        "#### 📊 今日宏觀焦點\n"
