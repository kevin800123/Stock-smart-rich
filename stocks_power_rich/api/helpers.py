...
def refresh_ssf_daily(c, day=None) -> dict:
    ...
    **`already_done` 的定義是「D 已經寫入」（`ssf_ready:{D}` 這個 marker 存在），不是
    「`ssf_daily` 裡剛好已經有 D 這個列」**（review I2／Fix C）——排程抓的是 `[D-6, D]`
    約 7 個日曆天的重疊區間，D 的前幾個交易日幾乎必定回得來，即使 D 自己的日盤資料
    還沒發佈；若只看 `get_ssf_dates` 判斷已完成，`/api/ssf/backfill` 或別的來源先把
    D 的列寫進去時會被誤判成「排程已確認過 D 到齊」而跳過重新確認。`ssf_ready:{D}`
    只在 D 真的被寫入時才寫，記下**第一次**成功的時間（沿用自算選股 `ready_at` 的
    既有模式）：之後同一天再呼叫都回報同一個時間，這樣 `/api/health` 的
    `jobs.ssf_daily.note` 才能回答「D 是哪個時段第一次到齊」，不必事後用猜的
    （`note` 是 `run_job` 把這個 dict 字串化存進去的結果，不是巢狀物件，要在字串裡
    找 `ready_at` 那一段，不能用 `note.ready_at` 這種路徑取值）。
    ...
    先拿到資料」這個既有的量測方法完全失效——D 沒發佈、但重疊區間裡前幾天仍抓得到
    資料時，正是這個分辨要防住的情境。
...
def job_schedule(cfg, schedule_time: str) -> list[dict]:
    ...
    specs = [
        ...
        # 股期概況：平日 17:15／18:15／20:15 各試一次。D 的日盤資料確切發佈時間仍未量到，
        # 只有兩個界線（2026-09-18 實測）：10:02 當天請求只回夜盤列（一般 0 列）；約 20:50
        # 本機回補已取得當天完整日盤資料——確切時間落在這個區間內，三次重試就是為了吸收
        # 這個未知；抓取器的守衛會擋掉「只有夜盤」的半套資料，所以早試不會寫壞。之後要量，
