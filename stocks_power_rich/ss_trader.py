...
# 固定刻度對不上實際分布時整個檢核項會永遠只吐同一類（實測 42 天全判「偏熱」）。
# 改用「相對損益兩平」——剛融資買進、價格未動時 市值÷融資金額 = 1/融資成數，
# 所以兩平線可由成數直接推出，是定義上就成立的錨點，不必借外部數字。
# 2026-09 起判讀的對象是證交所公布的全市場整戶擔保維持率（上市上櫃同一個數字），
# 兩平線用兩市場融資金額加權出的單一成數推得（blended_margin_ratio），不再分市場各一條。
...
def margin_verdict(maintenance, ratio: float) -> tuple[str, float, str]:
    """整戶擔保維持率 → (status, 相對兩平%, 說明)。

    ratio 是 blended_margin_ratio 以兩市場融資金額加權出的全市場成數，兩平線由它推得；
    判讀的對象是證交所公布的全市場整戶維持率（上市上櫃同值），只有這一條線。
...
# 上面兩個是「相對」量能（今天 vs 最近幾天）；下面這條是「絕對」水位，兩者互補。
# 10日均量 ≤ 8000 億＝上市量能深度萎縮。門檻取自黃國華常提的量能觀察值，
# 用觀察語意命名（QUIET）而非動作語意（BUY）——全站只陳述觀察、不給進出場動作。
# 註：本站 turnover 為上市（TWSE FMTQIK）口徑，與其他來源的上市櫃合計數不可直接對照。
VOL_MA_DAYS = 10            # 均量天數
MARGIN_CHG_DAYS = 5         # 融資餘額增減的比較窗口（交易日）——「融資 vs 大盤」看的是速度
VOL_QUIET_YI = 8000.0       # 10日均量 ≤ 此值＝量能深度萎縮
