...
9. **每日財經新聞**：台股／美股 Google 新聞白名單＋日股株探（kabutan.jp）標題，Gemini 統整後可在頁面閱讀或推送 Telegram；株探版面失效時會標示改用 Google 日經新聞備援。
10. **跨週 ＋ AI**：本週 vs 上週（新進榜／加速／退榜）＋ AI 籌碼分析師。
11. **設定**：每日排程時間、「讀取最新檔」資料夾、資料庫狀態、Gemini／LINE／Telegram 狀態（**只顯示已/未設定，不顯示也不輸入金鑰**）、盤中哨兵是否只警示交集股、單筆可容忍虧損（供杯柄頁算建議部位）、左側分頁順序（可拖拉、雲端同步）。
...
| `GET /api/news`、`POST /api/news/test` | 每日財經新聞摘要／手動推送 Telegram（摘要快取按日期＋時段分開） |
...
2. **環境變數**：
   ...
   | `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | （token / chat id） | 每日財經新聞推播（選用，兩者皆需） |
